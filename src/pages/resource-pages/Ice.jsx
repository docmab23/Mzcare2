...
function Ice() {
  ...
  const { iceJson, iceList, setICEJson, setICEList } =
    useDatabase();
  ...
  return (
    <IonPage>
      <IonContent className="ion-padding">
		...
		{iceList.length === 3 ? (
			<IonButton disabled={true} id="open-modal" expand="block" onClick={changestatus}>
			Add Contact
		  </IonButton>
		) : (
			<IonButton disabled={false} id="open-modal" expand="block" onClick={changestatus}>
          Add Contact
        </IonButton>
		) }
		...
		<IonModal isOpen={editStatus}>
          <IonHeader>
            <IonToolbar>
              <IonButtons slot="start">
                <IonButton onClick={changeEditStatus}>Save</IonButton>
              </IonButtons>
